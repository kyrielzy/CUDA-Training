...
//   temp2 = (float*)malloc(size);
  ...
  for (istep=0; istep < nstep; istep++) {
    ...
    // 对于如何合适的设置线层数量还是不太理解
    dim3 threadPerBlocks(32,16,1);
    dim3 numOfBlocks((nj/threadPerBlocks.x)+1,(ni/threadPerBlocks.y)+1,1);
  // Execute the modified version using same data
  for (istep=0; istep < nstep; istep++) {
    step_kernel_mod<<<numOfBlocks,threadPerBlocks>>>(ni, nj, tfac, temp1, temp2);
